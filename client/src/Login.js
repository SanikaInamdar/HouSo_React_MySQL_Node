...
function LoginPage() {
  ...
  const ownerlogin = () => {
    Axios.post('http://localhost:3001/ownerlogin', {
      username: username,
      password: password,
    }).then((response) => {
      if (response.data.message) {
    ...
    return (
        ...
        <h1 className="login_heading">Login</h1>
          ...
          <button className="login_button1" onClick={adminlogin}>Society Admin</button>
          <button className="login_button2" onClick={ownerlogin}>Flat Owner</button>
        </div>
        </div>
    );
}
